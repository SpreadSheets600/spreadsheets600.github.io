export const badgeFlags: Record<string, number> = {
  STAFF: 1 << 0,
  PARTNER: 1 << 1,
  HYPESQUAD: 1 << 2,
...
export const badgeInfo: Record<string, { label: string; icon: string | null }> = {
  STAFF: {
    label: "Discord Staff",
...
export function decodeUserFlags(flags: number) {
  const badges = [];
...
export async function fetchUserImages(data: any) {
  ...
  const defaultAvatarIndex = discord_user.discriminator === "0"
  ...
  const clanBadge = discord_user.primary_guild?.identity_guild_id && discord_user.primary_guild.badge
  ...
  const mainActivity = activities?.find((act: any) => act.type === 0);
