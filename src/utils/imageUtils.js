...
const fallbackBadgeIcon = UnknownIconDark ? `data:image/png;base64,${UnknownIconDark}` : null;
...
export async function fetchUserImages(data) {
	...
	const avatarUrl = discord_user.avatar ? `https://cdn.discordapp.com/avatars/${discord_user.id}/${discord_user.avatar}.${avatarExtension}?size=256` : `https://cdn.discordapp.com/embed/avatars/${defaultAvatarIndex}.png?size=256`;
	const avatar = await toDataUrl(avatarUrl);

	const decorationUrl = discord_user.avatar_decoration_data?.asset ? `https://cdn.discordapp.com/avatar-decoration-presets/${discord_user.avatar_decoration_data.asset}.png?size=128&passthrough=true` : null;
	const avatarDecoration = decorationUrl ? await toDataUrl(decorationUrl) : null;

	const clanBadgeUrl = discord_user.primary_guild?.identity_guild_id && discord_user.primary_guild.badge ? `https://cdn.discordapp.com/clan-badges/${discord_user.primary_guild.identity_guild_id}/${discord_user.primary_guild.badge}.png?size=32` : null;
	...
	const getActivityKey = (activity, index) => activity?.id || `${activity.application_id || "activity"}-${index}`;
	...
	const mainKey = mainActivity ? getActivityKey(mainActivity, activities.indexOf(mainActivity)) : null;
	...
	const spotifyAlbumArt = spotify?.album_art_url ? await toDataUrl(spotify.album_art_url) : null;
